...
Apr 4 & 1.1 & Changes to match work done and current results\\
\bottomrule
\end{tabularx}
...
\textbf{Expected Output:} Successfully demonstrated to the participants the following:
\begin{itemize}
\item Server attempts to restart when it crashes
\end{itemize}

\textbf{Actual Output:} Successfully demonstrated to the participants that the server is set to restart once it goes down
...
\textbf{Actual Output:} Successfully demonstrated to the participants that the current plan has enough space for expected number of users
...
\textbf{Actual Output:} Successfully demonstrated to the participants that adding a new building is just like adding a new scene and will not affect the speed of the application
...
\textbf{Actual Output:} Successfully demonstrated to the participants that the application is robust enough to work without major malfunctions for a long time and the finalized product will remain compatible with recent promised operating systems and devices.
...
\textbf{Actual Output:} demonstrated to the participants that the personal data we collect is handled legitimately
...
\textbf{Actual Output:} demonstrated to the participants that the authentication system and database will clean inactive data every semester
...
\textbf{Actual Output:} Successfully demonstrated to the participants that the user personal information is collected under permission only
...
\textbf{Actual Output:} Successfully demonstrated to the participants that the data send from/to the server are encrypted
...
\textbf{Actual Output:} Successfully demonstrated to the participants the following:
\begin{itemize}
\item The data collected will be handled as per the same legal requirements for the university
\item The application can abide by the guidelines set by university staff
\end{itemize}
...
There does not appear to be an observable trend. We suspected this was due to limitations in our testing setup. Upon further investigation we discovered that our Azure app server has a limit of around 2000 connections at the basic tier which is the tier we used.
We also discovered that JMeter could not create more than 5000 connections per IP. We concluded that a workaround for these limitations would be out of scope. It would both require paying for a higher tier of Azure service and the creation of a custom distributed JMeter testing setup which would be infeasible in our timeframe.
...
\item Clean: Pass, Firebase Authentication support some cloud functions to clean inactive users
...
We changed all the inconsistant naming regarding `pin' to `bookmark', making the icon and naming consistant and easier to understand intuitively.
...
These results are visible from the \href{https://github.com/beatlepie/4G06CapstoneProjectTeam2/actions/workflows/unit-tests.yml}{GitHub Actions}, where all automated testing results are visible.

\subsection{Testing Instruments}

The tests were performed using Unity Testing Framework, NUnit made for Unity.
The test files are located in multiple files with the respective names of the modules under \href{https://github.com/beatlepie/4G06CapstoneProjectTeam2/tree/main/test/CampusConnectionsTest/Assets/Editor}{test/CampusConnectionsTest/Assets/Editor}.
This was to separate the code from the packages that could not be loaded automatically.
