...
\item [\refstepcounter{mnum} \mthemnum \label{mMapInterface}:] MapInterface Module
...
\item [\refstepcounter{mnum} \mthemnum \label{mAccount}:] Account Module
\item [\refstepcounter{mnum} \mthemnum \label{mPermission}:] Permission Module
...
\subsubsection{MapInterface Module (\mref{mMapInterface})}
\begin{description}
...
\begin{description}
\item[Secrets:]How user-to-user chat session is established.
\item[Services:]Establishes a connection between two users to send message to each other.
\item[Implemented By:] CampusConnections
\item[Type of Module:] Library
\end{description}
