...
Apr 4  & 1.1 & Revision 1: Updated MIS to be consisted with design changes, implemented feedback, added new modules\\
\bottomrule
\end{tabularx}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
Initialize & (String, \textlangle3D Objects\textrangle) & - & -\\
Display & String & - & -\\
...
The $gps$ Environment Variable automatically updates $usrLoc$.

\subsubsection{Access Routine Semantics}

\noindent StartConnection($a$, $e$):
\begin{itemize}
\item transition: Opens a connection with $locationServer$ with address $a$ and endpoint $e$.
\item output: none
\item exception: none
\end{itemize}
...
\begin{itemize}
\item transition: Sends local user location $l$, obtained from the $gps$, to $locationServer$.
\item output: none
\item exception: none
\end{itemize}
...
UserLocation = tuple of (latitude: float, longitude: float, email: string)
